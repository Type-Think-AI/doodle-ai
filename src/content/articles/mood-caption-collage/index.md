---
title: "How to Make a Mood-Caption Collage from a Photo"
...
heroImage: "https://cdn.picxstudio.com/api/edited/image_4010b511-ada7-40b2-8dfd-a9cf2f2ca098.png"
category: "guide"
cluster: "social"
---
...
![A PicX-generated six-panel mood-caption doodle collage in a 3-by-2 landscape grid, each panel showing the same illustrated person with a short hand-lettered mood word](https://cdn.picxstudio.com/api/edited/image_4010b511-ada7-40b2-8dfd-a9cf2f2ca098.png)
...
```mermaid
flowchart TD
  Start[You have one photo and want several vibes] --> Q{What has to be true of the still?}
  Q -->|Six close-up faces, no type| Collage[Close-up collage]
  Q -->|Six head-to-toe actions, no type| FullBody[Full-body action collage]
  Q -->|Six moods with readable words on the page| Mood[Mood Captions]
  Q -->|One square portrait| Normal[Normal doodle avatar]
  Q -->|A sheet that looks peelable| Stickers[Stickers die-cut sheet]
  Q -->|One greeting-style message| Gift[Gift]
  Mood --> Live[Current job in this article]
```
...
flowchart TD
  ...
  Grid -->|No, 2x3 portrait or a sticker sheet| FailA[Treat as a miss: wrong object]
  Grid -->|Yes| Words{Six short captions, one per panel?}
  Words -->|No type, or extra slogans| FailB[Treat as a miss: collage or clutter]
  Words -->|Yes| Read{Readable at phone-feed size, off the face?}
  Read -->|No| FailC[Treat as a miss: lettering]
  Read -->|Yes| Same{Hair, face, outfit still one character?}
  Same -->|No| FailD[Treat as a miss: identity split]
  Same -->|Yes| Keep[Download or save, then crop in the destination app]
```
