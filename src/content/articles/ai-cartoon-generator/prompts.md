---
title: "AI Cartoon Generator Prompts: A Practical Hand-Drawn Doodle Prompt Library"
...
```mermaid
flowchart TD
  U[Your chat message] --> A[Mastra agent picks exactly one skill]
  Photo[Attached subject photo] --> T[generateDoodle]
  Ref[Optional style reference image] --> T
  A --> Skill{Which skill?}
  Skill -->|Normal Collage Full-body Stickers Mood Captions| Baked[Skill recipe: layout, pose pool, house style]
  Skill -->|Gift| Occ[Occasion keyword scan]
  Skill -->|Surprise| Brief[Your character brief, or a random seed]
  Baked --> T
  Occ --> T
  Brief --> T
  T --> PicX[Server-owned PicX still]
...
```mermaid
flowchart TD
  Wrong[The still is not what you wanted] --> Kind{Which failure?}
  Kind -->|Over-detailed or two skills in one paragraph| Short[Rewrite as one pattern. One skill. One still]
  Kind -->|Photo skill, nothing attached| Attach[Attach a consented photo, or switch to Surprise]
  Kind -->|Likeness drift or missing landmark| Lock[Same photo. Repeat 2-3 landmarks. One repair]
  Short --> Run[Spend 1 credit if it runs]
  Attach --> Run
  Lock --> Run
  Run --> Check{Human check against the photo}
  Check -->|Keeper| Save[Download or save to a moodboard]
  Check -->|API error| Refund[Credit refunds. Fix input, retry]
  Check -->|Succeeded but worse| Stop[Stop. Taste is not a refund]
```
