...
```mermaid
flowchart TD
  Start[You typed AI cartoon generator] --> Input{What do you already have?}
  Input -->|A photo of a real person or pet| Photo[Photo-to-cartoon still]
  Input -->|A written description and no likeness photo| Text[Text-to-image cartoon still]
  Input -->|A script, lesson, or explainer story| Script[Script-to-animation video]
  Input -->|A clip that must move, speak, or loop| Video[Video generation]
  Photo --> Doodle[Current Doodle AI job]
  Text --> Surprise[Doodle Surprise skill if you want a fictional doodle still]
  Script --> Other[Not a Doodle AI job]
  Video --> Other
```
...
sequenceDiagram
  ...
  participant Tool as generateDoodle
  participant PicX as Server-owned PicX
  You->>Browser: Chat turn and optional photo
  Browser->>Agent: Signed-in message plus attachment URLs
  Agent->>Agent: Pick one skill
  Agent->>Tool: Skill id and photo URL if required
  Tool->>Tool: Reserve 1 pooled credit
  Tool->>PicX: Still-image generation
  alt PicX returns a still
    PicX-->>Tool: Hosted image
    Tool-->>Agent: Success
    Agent-->>Browser: Still rendered in chat
  else Generation fails
    PicX-->>Tool: Failure
    Tool->>Tool: Refund the reserved credit
    Tool-->>Agent: Error status
    Agent-->>Browser: Plain-language failure
  end
```
