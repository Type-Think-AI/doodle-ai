...
flowchart TD
  ...
  G --> H[1 credit reserved]
  H --> I{PicX returns a still?}
  I -->|Yes| J[Square doodle appears in chat]
  I -->|No| K[Credit refunded]
  J --> L[Download, save to a moodboard, or ask for a refinement]
  K --> M[Fix the photo or the prompt, then try again]
```
...
```mermaid
flowchart TD
  A[Signed-in generation request] --> B{Rate limit ok?}
  B -->|No| C[No ledger write. Wait and retry]
  B -->|Yes| D{Photo attached for Normal?}
  D -->|No| E[Needs-photo. No credit spent]
  D -->|Yes| F{Organization has 1 credit?}
  F -->|No| G[Insufficient credits. Nothing generated]
  F -->|Yes| H[Reserve 1 credit and mark generation pending]
  H --> I[PicX image edit with the uploaded photo]
  I -->|Hosted image URL| J[Keep the spend. Status ok]
  I -->|Error or no URL| K[Refund 1 credit]
```
...
```mermaid
flowchart LR
  subgraph Phone[Your browser]
    P[Photo you chose]
    N[No PicX key in Settings]
  end
  subgraph Doodle[Doodle AI server]
    U[Authenticated upload]
    A[Mastra agent picks Normal]
    L[Credit reserve]
  end
  subgraph PicX[PicX managed assets]
    M[Hosted source photo]
    G[Generated 1:1 doodle]
  end
  P --> U
  U --> M
  A --> L
  L --> G
  G --> Phone
  N -.->|credentials never leave the server| Doodle
```
