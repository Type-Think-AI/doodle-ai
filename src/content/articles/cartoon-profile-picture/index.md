...
```mermaid
flowchart LR
  A[1:1 doodle still from Normal] --> B[You download the file]
  B --> C[Destination app applies a circular mask]
  C --> D[Tiny chat, comment, or member-list size]
  C --> E[Larger profile-page size]
  D --> F{Face, hair silhouette, and glasses still read?}
  E --> F
  F -->|Yes| G[Keep it as the PFP]
  F -->|No| H[Regenerate with a tighter crop or thicker outline]
```
...
```mermaid
flowchart TD
  P[Same consented selfie] --> W[Prompt: weekday, closed mouth]
  P --> G[Prompt: game-night grin]
  P --> N[Prompt: quieter, optional headphones]
  W --> W1[Still 1]
  G --> G1[Still 2]
  N --> N1[Still 3]
  W1 --> H{Human check: still Rae after a circle crop?}
  G1 --> H
  N1 --> H
  H -->|Yes| K[Download and crop in Instagram, Discord, or Reddit]
  H -->|No, landmark missing| R[One repair prompt, then stop if it is worse]
  H -->|No, invented prop ate the hair| X[Drop the prop. Do not treat this as a matching set]
```
