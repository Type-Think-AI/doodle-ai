...
```mermaid
flowchart TD
  Start[Open the camera roll] --> Count{How many pets fill the frame?}
  Count -->|Two or more overlapping| Recrop[Crop to one animal or pick another photo]
  Count -->|One animal| Light{Can you see both eyes and the coat markings?}
  Light -->|No: dark, backlit, or muddy| Relight[Move to a window or open shade and reshoot]
  Light -->|Yes| Sharp{Is the face sharp, not a leap-smear?}
  Sharp -->|No| Still[Wait for a sit, a nap, or a treat pause]
  Sharp -->|Yes| Rights{Do you have the right to use this photo?}
  Rights -->|No| Stop[Do not upload it]
  Rights -->|Yes| Ready[This is a usable pet-portrait input]
  Recrop --> Light
  Relight --> Sharp
  Still --> Light
```
...
flowchart TD
  ...
  Job --> Card{Something you can send to a person}
  Job --> Sheet{Several peel-apart poses on one page}
  Face --> Normal[Normal doodle avatar, 1:1]
  Card --> Gift[Gift doodle, 1:1 greeting still]
  Sheet --> Stickers[Sticker Pack, square die-cut sheet]
  Normal --> Keep[Download or save the digital still]
  Gift --> Keep
  Stickers --> Keep
  Keep --> PrintNote[Printing later is your errand, not a Doodle AI shipment]
```
