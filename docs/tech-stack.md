...
| **Secrets** | `wrangler secret put` for `PICX_API_KEY` and `OPENROUTER_API_KEY` | The platform-owned PicX key is the only provider credential used for generation. Note `src/lib/env-bridge.ts` already handles the Workers-env → `process.env` bridge that Mastra needs. |
